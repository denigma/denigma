...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Link.contact'
        db.add_column('links_link', 'contact',
                      self.gf('django.db.models.fields.CharField')(max_length=100, null=True, blank=True),
                      keep_default=False)

        # Adding M2M table for field contacts on 'Link'
        db.create_table('links_link_contacts', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('link', models.ForeignKey(orm['links.link'], null=False)),
            ('profile', models.ForeignKey(orm['experts.profile'], null=False))
        ))
        db.create_unique('links_link_contacts', ['link_id', 'profile_id'])


    def backwards(self, orm):
        # Deleting field 'Link.contact'
        db.delete_column('links_link', 'contact')

        # Removing M2M table for field contacts on 'Link'
        db.delete_table('links_link_contacts')
