...
urlpatterns = patterns('expressions.views',
    ...
    # Profiles:
    url('^profiles/$', 'profiles', name='profiles'), #login_required()
    ...
    url('^profile/create/$', ProfileCreate.as_view(),
        name='create-profile'),
    ...
    url('^intersections/table/(?P<ratio>[\d\.]+)/(?P<pvalue>[\d\.]+)/(?P<fold_change>[\w\d\.]+)/(?P<exp>[\w\d\.]+)/(?P<set>\d+)/(?P<benjamini>[\w\d\.]+)', 'intersections_table', name='intersection_table'),  # #}
