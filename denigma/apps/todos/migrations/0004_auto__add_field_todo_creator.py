...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Todo.creator'
        db.rename_column('todos_todo', 'owner_id', 'creator_id')

        # Adding M2M table for field executor on 'Todo'
        db.create_table('todos_todo_executor', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('todo', models.ForeignKey(orm['todos.todo'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('todos_todo_executor', ['todo_id', 'user_id'])

        # Adding M2M table for field categories on 'Todo'
        db.create_table('todos_todo_categories', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('todo', models.ForeignKey(orm['todos.todo'], null=False)),
            ('entry', models.ForeignKey(orm['data.entry'], null=False))
        ))
        db.create_unique('todos_todo_categories', ['todo_id', 'entry_id'])


    def backwards(self, orm):
        # Deleting field 'Todo.creator'
        db.rename_column('todos_todo', 'creator_id', 'owner_id')

        # Removing M2M table for field executor on 'Todo'
        db.delete_table('todos_todo_executor')

        # Removing M2M table for field categories on 'Todo'
        db.delete_table('todos_todo_categories')
