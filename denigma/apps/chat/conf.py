...
from chat import __version__, __version_string__, __url__
...
options.add_option("-C", "--irc-channel", dest="IRC_CHANNEL",
    metavar="CHANNEL", default="#denigma",
...
options.add_option('-l', '--log-level', dest='LOG_LEVEL', metavar='INFO|DEBUG',
                    ...
                    help='Log level to use. DEBUG will spwe out all IRC data. '
                        'default: %default]')
...
class Settings(dict):
    """Settings object, backed by either Django settings, or CLI args.
    ...
    def __init__(self):
        ...
        self['CHAT_VERSION'] = __version__
        self['CHAT_VERSION_STRING'] = __version_string__
        self['CHAT_PROJECT_URL'] = __url__
        try:
            from django.conf import settings
            for k, v in parser.defaults.items():
                self[k] = getattr(settings, 'CHAT_%s' % k, v)
    ...
    def parse_args(self):
        """Called from ``chat.server_run`` and parses any CLI args
        provided. Also handles loading settings from the Python module
        specified with the ``-conf-file`` arg. CLI args take precedence
        over any settings defined in the Python module defined by ``--conf-file``."""
        ...
        for option in self.option_list:
            if option.dest:
                file_value = file_settings.get('CHAT_%s' % option.dest, None)
