...
def color(nickname):
    ...
    of nickname's md5 has, and then slighlty darkens the rgb values
...
class BaseIRCClient(SimpleIRCClient, object):
    ...
    def on_nicknameinuse(self, connection, event):
        """Increment a digit on the nickname if it is in use, and
        re-connect."""
        digits = ''
...
class WebSocketIRCClient(BaseIRCClient):
    """IRC client that is bridged with a gevent-socketio namespace."""
    ...
    def emit_nicknames(self):
        """Send the nickname list to the Websocket. Called whenever the nicknames
        list changes."""
    ...
    def on_nick(self, connection, event):
        ...
        old_color = self.nickname.pop(old_nickname)
        new_nickname = event.target()
        message = "is nwo known as %s" % new_nickname
    ...
    def on_quit(self, connection, event):
        """Someone left the channel -send the nicknames list to the WebSockets."""
        nickname = self.get_nickname(event)
