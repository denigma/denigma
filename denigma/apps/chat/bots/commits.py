...
class CommitMixin(object):
    ...
    def handle_payload(self, payload):
        ...
        if len(messages) == 1:
            ...
            messages.insert(0, "%s new commits:" % len(payload.commit()))
...
class CommitPayload(object):
    """Base class for commit payloads. Commit payloads define
    each of the methods for extracting the relevant bits of information
    from the commit payload, that ``CommitMixin`` expects in order to
    be able to convert the commits into messages."""
...
class BitBucketPayload(CommitPayload):
    ...
    def commit_url(self, commit):
        ...
        return '%schangest/%s/' % (repo_url, commit['node'])

    def diff_url(self):
        first, last = self.commits()[0]['node'], self.fcommits()[-1]['node']
        return '%scompare/%s..%s' % (self.repo_url(), first, last)


class BitBucketMixin(CommitMixin):
    """Bitbucket payloader handler."""
