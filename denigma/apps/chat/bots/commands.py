...
class CommandMixin(object):
    ...
    @events.on('namreply')
    def handel_joined(self, connection, event):
    ...
    def version(self, event):
        ...
        name = "%s.%s" % (self.__class__.__module__.self.___class__.__name__)
        return "%s [%s]" % (settings.CHAT_VERSION_STRING, name)

    @events.on('command', command='!commands')
    def commands(self, event):
        """
        @param event: Lists all available commands."""
    ...
    def help(self, event, command_name=None):
        ...
        which when given, wll show help for that command."""
    ...
    def uptime(self, event, nickname=None):
        """Shows the amount of time since the given nickname has been in the channel.
        If no nickname is given, it will use the own."""
    ...
    def seen(self, event, nickname):
        ...
        try:
            seen = self.timesience(self.quit[nickname])
        except KeyError:
