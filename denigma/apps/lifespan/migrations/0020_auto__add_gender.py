...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Gender'
        db.create_table('lifespan_gender', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('name', self.gf('django.db.models.fields.CharField')(max_length=13)),
        ))
        db.send_create_signal('lifespan', ['Gender'])

        # Adding M2M table for field gender on 'Measurement'
        db.create_table('lifespan_measurement_gender', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('measurement', models.ForeignKey(orm['lifespan.measurement'], null=False)),
            ('gender', models.ForeignKey(orm['lifespan.gender'], null=False))
        ))
        db.create_unique('lifespan_measurement_gender', ['measurement_id', 'gender_id'])

        # Adding M2M table for field gender on 'Intervention'
        db.create_table('lifespan_intervention_gender', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('intervention', models.ForeignKey(orm['lifespan.intervention'], null=False)),
            ('gender', models.ForeignKey(orm['lifespan.gender'], null=False))
        ))
        db.create_unique('lifespan_intervention_gender', ['intervention_id', 'gender_id'])


    def backwards(self, orm):
        # Deleting model 'Gender'
        db.delete_table('lifespan_gender')

        # Removing M2M table for field gender on 'Measurement'
        db.delete_table('lifespan_measurement_gender')

        # Removing M2M table for field gender on 'Intervention'
        db.delete_table('lifespan_intervention_gender')
