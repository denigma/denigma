...
#         login_required(InterventionDelete.as_view())),
    ...
    url(r'^factor/create/$', login_required(CreateFactor.as_view()), name='create-factor'),
   ...
   # url(r'^variants/(?P<chromosome>.+)?', VariantList.as_view(), name='variants'),
    ...
    url(r'^variance/(?P<name>.+)/$', VarianceDetail.as_view(), name='variant-detail'),
    ...
    url(r'^variant_types/$', ListView.as_view(queryset=VariantType.objects.all()), name='variant_types'),
    url(r'^or_types/$', ListView.as_view(queryset=ORType.objects.all()), name='or_types'),
