Denigma.Graph.webglLabelNodeProgram = (defLabel)->
  ATTRIBUTES_PER_PRIMITIVE = 18
  nodesFS = ["precision mediump float;",
             "varying vec4 color;",
             ...
             "void main(void) {", "   if (vTextureCoord.z == 0.) {",
             "     gl_FragColor = texture2D(u_sampler0, vTextureCoord.xy);",
             "   } else if (vTextureCoord.z == 1.) {", "     gl_FragColor = texture2D(u_sampler1, vTextureCoord.xy);",
             "   } else if (vTextureCoord.z == 2.) {", "     gl_FragColor = texture2D(u_sampler2, vTextureCoord.xy);",
             "   } else if (vTextureCoord.z == 3.) {", "     gl_FragColor = texture2D(u_sampler3, vTextureCoord.xy);",
             "   } else { gl_FragColor = vec4(0, 1, 0, 1); }", "}"].join("\n")
  nodesVS = ["attribute vec2 a_vertexPos;",
             ...
             "varying vec3 vTextureCoord;",
             "void main(void) {", "   gl_Position = u_transform * vec4(a_vertexPos/u_screenSize, 0, 1);",
             ...
             "if(corner == 0.0) {", "  vTextureCoord.xy = vec2(0, 1);",
             "} else if(corner == 1.0) {",
             "  vTextureCoord.xy = vec2(1, 1);", "} else if(corner == 2.0) {",
             "  vTextureCoord.xy = vec2(0, 0);", "} else {",
  ...
  transform = undefined
  sizeDirty = undefined
  refreshTexture = (texture, idx) ->
    gl.deleteTexture texture.nativeObject  if texture.nativeObject
    nativeObject = gl.createTexture()
  ...
  ensureAtlasTextureUpdated = ->
    if atlas.isDirty
      ...
      while i < textures.length
        refreshTexture textures[i], i  if textures[i].isDirty or not textures[i].nativeObject
        ++i
      atlas.clearDirty()
  ...
  load: (glContext) ->
    ...
    atlas = new ImageAtlas(tilesPerTexture)
  ...
  ###
  position: (nodeUI, pos) ->
    idx = nodeUI.id * ATTRIBUTES_PER_PRIMITIVE
  ...
  createNode: (ui) ->
    ...
    nodesCount += 1
    coordinates = atlas.getCoordinates(ui.src)
    if ui.text?
      text = ui.text
    else
      text = defLabel
    if coordinates
      ...
      atlas.load ui.src,text, (coordinates) ->
        ui._offset = coordinates.offset


  removeNode: (nodeUI) ->
    nodesCount -= 1  if nodesCount > 0
    if nodeUI.id < nodesCount and nodesCount > 0
      atlas.remove nodeUI.src  if nodeUI.src
